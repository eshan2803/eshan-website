"""
Energy breakdown visualization V2 with import sources classified.

Panel 1: Daily energy (MWh) stacked area chart (Fossil vs Clean, no imports category)
...
# Load daily energy data with import breakdown
print("Loading daily energy breakdown with import classification...")
...
for date_str in sorted(daily_data.keys()):
    ...
    fossil_mwh.append(daily_data[date_str]["fossil_mwh"])
...
fossil_mwh = np.array(fossil_mwh)
...
# ══════════════════════════════════════════════════════════════════════════
# Panel 1: Daily energy stacked area chart (Fossil vs Clean)
# ══════════════════════════════════════════════════════════════════════════
...
fossil_gwh = fossil_mwh / 1000.0
clean_gwh = clean_mwh / 1000.0
demand_gwh = gross_demand_mwh / 1000.0

# Stacked area plot (bottom to top): Fossil, Clean
ax1.fill_between(x_dates, 0, fossil_gwh,
                 color="#f97316", alpha=0.8, label="Fossil (Gas + Fossil Imports + Unknown)", zorder=2)
ax1.fill_between(x_dates, fossil_gwh, fossil_gwh + clean_gwh,
                 color="#4ade80", alpha=0.8, label="Clean (Renewables + Nuclear + Hydro + Batteries + Clean Imports)", zorder=2)
...
last_date_label = sorted(daily_data.keys())[-1]
last_date_fmt = dt.strptime(last_date_label, "%Y-%m-%d").strftime("%B %d, %Y")
ax1.set_title(f"California Grid: Daily Energy Supply & Demand\n"
...
# Legend
ax1.legend(loc='upper left', fontsize=10, framealpha=0.9, facecolor=BG_INNER,
...
ax2.set_title("Daily Clean Energy Percentage (With Import Classification)\n"
              "Clean % = Clean Energy / (Fossil + Clean) × 100",
...
# Save
print("Saving figure...")
out_path = os.path.join(script_dir, "energy_breakdown_v2.png")
...
# Print comparison
print(f"\nKey Insight:")
print(f"  Average clean energy %: {clean_pct.mean():.1f}%")
print(f"  This is higher than the original chart (50.9%) because imports are now classified!")
print(f"  2024 imports were 70.5% clean, significantly raising overall clean %")
