...
last_data_date = min(sorted(price_data.keys())[-1], sorted(daily_peak_mw_raw.keys())[-1])
last_data_label = datetime.strptime(last_data_date, "%Y-%m-%d").strftime("%B %d, %Y")
fig.suptitle(f"Peak Electricity Price vs Battery Storage Capacity by Year\n"
             f"Updated through {last_data_label}  |  Color = Battery as % of Peak Demand",
