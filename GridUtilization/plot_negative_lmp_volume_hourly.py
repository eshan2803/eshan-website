"""
Negative LMP scatter charts — HOURLY resolution (2020-2026).
...
# ── Read CSV (hourly rows only) ──
print("Reading comprehensive CSV (hourly rows only)...")
...
with open(CSV_FILE, "r", encoding="utf-8") as f:
    ...
    for row in reader:
        ...
        # Only keep :00 rows for consistent hourly resolution
...
for year in sorted(negative_intervals.keys()):
    ...
    print(f"  {year}: {n:,} hours, min ${min_lmp:.2f}")
...
def make_chart(x_index, x_label, title_suffix, filename):
    ...
    for idx, (ax, year) in enumerate(zip(axes, YEARS)):
        ...
        ax.scatter(x_vals, lmps, c=DOT_COLOR, s=12, alpha=0.5,
        ...
        ax.text(0.96, 0.03, f"{n_hours:,} hrs\n{total_gwh:,.0f} GWh\nMin: ${min_price:.0f}",
    ...
    fig.suptitle(f"California Grid: Negative LMP Events — {title_suffix}\n"
                 f"Each dot = one hour with negative price (hourly resolution, 2020-2026)",
...
make_chart(1, "Grid Load (GWh)", "LMP vs Grid Load", "negative_lmp_volume_hourly.png")

print("\nChart 2: LMP vs Net Load...")
make_chart(2, "Net Load (GWh)", "LMP vs Net Load (Load − Solar − Wind)", "negative_lmp_net_load_hourly.png")
