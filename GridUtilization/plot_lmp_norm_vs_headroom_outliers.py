"""
Scatter plot: LMP normalized by CA Natural Gas Citygate price vs Effective Headroom.
OUTLIER VIEW — only shows points where LMP/GasPrice > 20 (top ~5% of hours).
...
Saves output as lmp_norm_vs_headroom_outliers.png.
"""
...
OUTLIER_THRESHOLD = 20  # Only show points with LMP/GasPrice > this value
...
for date_str in price_dates:
    ...
    for h in range(24):
        ...
        norm_lmp_values.append(norm_val)
        hours.append(h)
        headrooms.append(headroom)

print(f"Total hourly data points: {total_points}")
print(f"Outlier points (LMP/Gas > {OUTLIER_THRESHOLD}): {len(headrooms)} ({100*len(headrooms)/total_points:.1f}%)")
...
print(f"Normalized LMP range: {norm_lmp_values.min():.1f} to {norm_lmp_values.max():.1f}")
...
ax.set_title(f"LMP Normalized by CA Gas Citygate Price vs Effective Headroom — Outliers Only\n"
             f"CAISO Hourly Data: Jan 2020 – Dec 2025  |  LMP/Gas > {OUTLIER_THRESHOLD} ({len(headrooms):,} points)",
             color="#fff", fontsize=14, fontweight="bold", pad=15)

ax.set_ylim(OUTLIER_THRESHOLD, norm_lmp_values.max() * 1.02)
...
out_path = os.path.join(script_dir, "lmp_norm_vs_headroom_outliers.png")
