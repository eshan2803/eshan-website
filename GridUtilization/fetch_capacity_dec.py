...
def fetch_ca_capacity_for_period(period, offset=0, page_size=5000):
    """
    Fetch all operating generators in California for a given period (e.g. '2024-12').
...
def main():
    start_year = 2020
    end_year = 2024  # 2025-12 doesn't exist yet
    ...
    print("Snapshot: DECEMBER of each year (year-end capacity)")
    print("==============================================")

    # Fetch December of each year as the snapshot
    results = {}
    all_resources = set()
    ...
    for year in range(start_year, end_year + 1):
        ...
        wind = capacity.get('Wind', 0)
        print(f"  Total installed: {total:,.1f} MW across {len(capacity)} resource types.")
        print(f"  Wind: {wind:,.1f} MW")
        time.sleep(2)

    # For 2025, use the most recent available month (likely November or December 2024)
    print(f"\nFetching 2025 (using latest available snapshot)...")
    period = "2024-12"
    records = fetch_ca_capacity_for_period(period)
    if records:
        capacity = aggregate_by_resource(records)
        results["2025"] = capacity
        all_resources.update(capacity.keys())
        total = sum(capacity.values())
        wind = capacity.get('Wind', 0)
        print(f"  Total installed: {total:,.1f} MW (using {period} snapshot)")
        print(f"  Wind: {wind:,.1f} MW")
    ...
    print("\n--- Wind Capacity Trend ---")
    for year in sorted(results.keys()):
        wind = results[year].get('Wind', 0)
        print(f"  {year}: {wind:,.1f} MW")
