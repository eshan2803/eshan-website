"""
Scatter plot: MEC (energy component of LMP) normalized by CA Natural Gas Citygate price vs Effective Headroom.
OUTLIER VIEW — only shows points where MEC/GasPrice > 20 (top ~5% of hours).

Y-axis = MEC ($/MWh) / Gas Price ($/MCF) → units: MCF/MWh (implied heat rate proxy)
X-axis = Effective Headroom = Available Gas Capacity - Net Load (GW)

Uses hourly data for all dates with LMP data (2020-01-01 onwards).
Saves output as mec_norm_vs_headroom_outliers.png.
"""
...
OUTLIER_THRESHOLD = 20  # Only show points with MEC/GasPrice > this value
...
for date_str in price_dates:
    ...
    for h in range(24):
        ...
        total_points += 1
        norm_val = mec / gas_price
        if norm_val < OUTLIER_THRESHOLD:
            continue
        ...
        norm_mec_values.append(norm_val)
        hours.append(h)
        headrooms.append(headroom)

print(f"Total hourly data points: {total_points}")
print(f"Outlier points (MEC/Gas > {OUTLIER_THRESHOLD}): {len(headrooms)} ({100*len(headrooms)/total_points:.1f}%)")

norm_mec_values = np.array(norm_mec_values)
hours = np.array(hours)
headrooms = np.array(headrooms)

print(f"Headroom range: {headrooms.min():.0f} to {headrooms.max():.0f} MW")
print(f"Normalized MEC range: {norm_mec_values.min():.1f} to {norm_mec_values.max():.1f}")
...
# Styling
ax.set_xlabel("Effective Headroom (GW)\nAvailable Gas Capacity minus Net Load",
              color="#ccc", fontsize=12, labelpad=10)
ax.set_ylabel("MEC / Gas Price (MWh\u207b\u00b9 \u00b7 MCF)", color="#ccc", fontsize=12, labelpad=10)
ax.set_title(f"MEC Normalized by CA Gas Citygate Price vs Effective Headroom — Outliers Only\n"
             f"CAISO Hourly Data: Jan 2020 \u2013 Dec 2025  |  MEC/Gas > {OUTLIER_THRESHOLD} ({len(headrooms):,} points)",
             color="#fff", fontsize=14, fontweight="bold", pad=15)

ax.set_ylim(OUTLIER_THRESHOLD, norm_mec_values.max() * 1.02)
...
ax.axvline(x=0, color="#ef4444", linewidth=0.8, linestyle="--", alpha=0.6)

plt.tight_layout()

out_path = os.path.join(script_dir, "mec_norm_vs_headroom_outliers.png")
