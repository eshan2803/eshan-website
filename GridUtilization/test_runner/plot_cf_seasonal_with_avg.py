...
with average capacity factor overlaid on each seasonal column.
"""
import json
import time
import numpy as np
...
def color_sorted_jitter(cfs, hours, x_centre, half_width, n_bins=200):
    ...
    # Bin edges spanning the data range
    cf_min, cf_max = cfs.min(), cfs.max()
    bin_edges = np.linspace(cf_min - 1e-9, cf_max + 1e-9, n_bins + 1)
    bin_idx = np.digitize(cfs, bin_edges) - 1  # 0-based bin index

    # Compute max density ONCE (was inside the loop before — O(n*n_bins) → O(n))
    ...
    for b in range(n_bins):
        ...
        # Width at this CF level from KDE (use pre-computed density)
        density_here = densities[b]
        width = (density_here / d_max) * half_width if d_max > 0 else half_width

        # Sort points in this band by hour so similar colours are adjacent
        indices = np.where(mask)[0]
        order = np.argsort(hours[indices])
        sorted_indices = indices[order]

        n = len(sorted_indices)
        # Spread evenly from -width to +width
...
for i, fuel in enumerate(fuel_order):
    for j, season in enumerate(SEASON_ORDER):
        ...
        x_pos = color_sorted_jitter(cfs, hours, x_centre, max_half_strip)
        ...
        # ── Average CF overlay ────────────────────────────────────────
        mean_cf = cfs.mean()
        half_w = max_half_strip * 0.85
        ax.hlines(mean_cf, x_centre - half_w, x_centre + half_w,
                  colors="white", linewidths=1.5, zorder=5)
        ax.text(x_centre, mean_cf + 0.015, f"{mean_cf:.2f}",
...
ax.set_title("Hourly Capacity Factor by Generator Type & Season  ",
...
plt.tight_layout()
plt.savefig("capacity_factor_seasonal_with_avg.png", dpi=200,
            bbox_inches="tight", facecolor=BG_COLOR)
print("Saved capacity_factor_seasonal_with_avg.png")
