"""
Battery storage discharge as % of demand (daily peak, white line)
...
with open("caiso_battery_daily_peak.json") as f:
    daily_peak_raw = json.load(f)

with open("caiso_prices.json") as f:
    price_data = json.load(f)

# ── Daily peak battery % (pre-computed from CAISO supply data) ───────────
peak_bat_dates = np.array([datetime.strptime(d, "%Y-%m-%d") for d in sorted(daily_peak_raw.keys())])
peak_bat_vals  = np.array([daily_peak_raw[d] for d in sorted(daily_peak_raw.keys())])
print(f"Daily peak battery values: {len(peak_bat_vals):,} days")
...
print(f"Daily peak LMP range: {peak_lmp_vals.min():.1f} to {peak_lmp_vals.max():.1f} $/MWh")
...
# ── Left axis: daily peak battery % as white line ──────────────
ax1.plot(peak_bat_dates, peak_bat_vals,
         color="#ffffff", linewidth=1.2, alpha=0.9, zorder=5)
ax1.set_xlim(peak_bat_dates[0], peak_bat_dates[-1])
ax1.set_ylim(0, peak_bat_vals.max() * 1.08)

ax1.set_ylabel("Daily Peak Battery Discharge (% of Demand)",
               fontsize=12, color="#ffffff", fontweight="bold")
ax1.tick_params(axis="y", colors="#ffffff")

# ── Right axis: daily peak LMP scatter, colored by month ────────────────
ax2 = ax1.twinx()
...
print(f"Plot: {time.time()-t0:.1f}s")
...
ax1.set_title(
    "Battery Storage vs Peak Electricity Price  (CAISO 2020-2025)",
...
# ── Colorbars ────────────────────────────────────────────────────────────
# Colorbar (month for scatter)
sm_season = plt.cm.ScalarMappable(cmap=season_cmap, norm=month_norm)
sm_season.set_array([])
cbar1 = fig.colorbar(sm_season, ax=[ax1, ax2], pad=0.01, aspect=30, fraction=0.015)
cbar1.set_label("Scatter: Month", fontsize=9, color="#f97316")
...
plt.savefig("battery_pct_demand.png", dpi=200,
            bbox_inches="tight", facecolor=BG_COLOR)
print("Saved battery_pct_demand.png")
