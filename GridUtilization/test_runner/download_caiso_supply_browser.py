...
async def download_day(page, date, retry_count=0):
    ...
    date_str_windows = date.strftime("%m/%d/%Y")  # Format: 01/01/2020 (with leading zeros)
    ...
    try:
        ...
        # Use jQuery datepicker API to reliably set the date
        date_str = date.strftime("%#m/%#d/%Y")
        
        await page.evaluate(f"""() => {{
            jQuery("#dateSupply").datepicker("setDate", "{date_str}");
            jQuery("#dateSupply").trigger("change");
        }}""")

        print(f"    Set date to: {date_str_windows}")

        # Press Enter on the input to trigger the chart reload
        await page.locator("#dateSupply").press("Enter")
        ...
        await page.evaluate(f"""() => {{
            jQuery("#dateRenewables").datepicker("setDate", "{date_str}");
            jQuery("#dateRenewables").trigger("change");
        }}""")

        print(f"    Set date to: {date_str_windows}")

        # Press Enter on the input to trigger the chart reload
        await page.locator("#dateRenewables").press("Enter")
        await page.wait_for_timeout(1000)
