...
last_data_date = min(sorted(daily_peak_mw_raw.keys())[-1], sorted(price_data.keys())[-1])
last_data_label = datetime.strptime(last_data_date, "%Y-%m-%d").strftime("%B %d, %Y")
ax1.set_title(
    f"Battery Storage (GW) vs Peak Electricity Price  |  Updated through {last_data_label}\n"
