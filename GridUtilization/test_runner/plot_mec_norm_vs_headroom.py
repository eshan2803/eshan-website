...
Saves output as mec_norm_vs_headroom.png.
"""
...
for date_str in price_dates:
    ...
    for h in range(24):
        ...
        norm_mec_values.append(mec / gas_price)
        hours.append(h)
        headrooms.append(headroom)

print(f"Total hourly data points: {len(headrooms)}")
...
ax.set_title("MEC Normalized by CA Gas Citygate Price vs Effective Headroom\nCAISO Hourly Data: Jan 2020 \u2013 Dec 2025",
             color="#fff", fontsize=15, fontweight="bold", pad=15)

ax.set_ylim(norm_mec_values.min(), norm_mec_values.max())
...
out_path = os.path.join(script_dir, "mec_norm_vs_headroom.png")
