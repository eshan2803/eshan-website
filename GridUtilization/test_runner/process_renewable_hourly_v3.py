"""
Process CAISO fuel-source CSVs to extract hourly clean energy penetration values (V3).
...
Output:
  - renewable_penetration_hourly_v3.json  {
      "YYYY-MM-DD HH": clean_pct
    }
"""
import os
import csv
...
OUT_FILE = os.path.join(os.path.dirname(__file__), "renewable_penetration_hourly_v3.json")
        ...
        with open(fpath, "r", newline="", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                if gross_demand_mw > 0:
                    ...
                    hourly_intervals[hour].append(clean_pct)
...
print(f"\nSaved hourly clean energy penetration data (V3) to {OUT_FILE}")
...
print(f"\nSummary:")
print(f"  Hours with >=100% clean energy: {hours_over_100:,} ({hours_over_100/len(sorted_data)*100:.2f}%)")
print(f"  Average clean energy penetration: {avg_penetration:.1f}%")
print(f"  Peak clean energy penetration: {max_penetration:.1f}%")
